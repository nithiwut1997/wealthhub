package com.nithiwut.wealthhub.assetprice.controller;

import com.nithiwut.wealthhub.assetprice.dto.request.CreateAssetPriceRequest;
import com.nithiwut.wealthhub.assetprice.dto.response.AssetPriceResponse;
import com.nithiwut.wealthhub.assetprice.service.AssetPriceService;
import jakarta.validation.Valid;
import lombok.RequiredArgsConstructor;
import org.springframework.http.HttpStatus;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.ResponseStatus;
import org.springframework.web.bind.annotation.RestController;

@RestController
@RequestMapping("/api/v1/assets/{assetId}/prices")
@RequiredArgsConstructor
public class AssetPriceController {
    private final AssetPriceService assetPriceService;

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public AssetPriceResponse createAssetPrice(
        @PathVariable Long assetId,
        @Valid @RequestBody CreateAssetPriceRequest request
    ) {
        return assetPriceService.createAssetPrice(assetId, request);
    }

    @GetMapping("/latest")
    public AssetPriceResponse getLatestAssetPrice(@PathVariable Long assetId) {
        return assetPriceService.getLatestAssetPrice(assetId);
    }
}
